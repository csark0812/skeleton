...
Live commands consume host model usage. The default one-pair run is a diagnostic smoke test, not a reliability sample; it records efficiency without asserting the repeated-sample 15% qualification. Use `--runs N` for repeated comparisons. See [the method and scoring rules](../docs/developer/efficacy.md).
